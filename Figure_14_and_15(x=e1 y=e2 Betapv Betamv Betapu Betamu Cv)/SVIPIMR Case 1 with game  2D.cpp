...
# define m 0.5
	...
	file1 << "Betapv=0.90 Betamv=0.50 Cv=0.01 x=e1,y=e2 " << ".csv";

	ofstream Data1(file1.str().c_str(), ios_base::out | ios_base::trunc);

	Data1 << "e1,e2,V,Rs,Rv,P" << endl;
	...
	while (i < times) {
       ...
       // Data1 << i << "," << S[i] << "," << V[i] << ", " << Ipu[i] + Ipv[i] << "," << Imu[i] + Imv[i] << "," << Rs[i] + Rv[i] << "," <<Ipu[i] + Ipv[i]+Imu[i] + Imv[i]<< endl;
        ...
        Data1 << e1 << "," << e2 << " ," << V[times] << ", " << Rs[times] <<"," <<Rv[times]<< "," << Rs[times]+Rv[times] <<  endl;
        cout << e1 << "," << e2 << " ," << V[times] << "," << Rs[times] <<","<<Rv[times]<<"," << Rs[times]+Rv[times] << endl;
		}
