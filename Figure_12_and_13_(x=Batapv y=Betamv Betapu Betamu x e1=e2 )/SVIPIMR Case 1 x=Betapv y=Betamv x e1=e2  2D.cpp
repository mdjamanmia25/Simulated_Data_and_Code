...
//# define m 0.5
	...
	file1 << "Betapu=0.90,Betamu=0.90 x=0.50 e1=e2=0.50 x=Betapv,y=Betamv" << ".csv";

	ofstream Data1(file1.str().c_str(), ios_base::out | ios_base::trunc);

	Data1 << "Betapv,Betamv,V,Rs,Rv,P" << endl;
	...
	while (i < times) {
      ...
      //  double xx = X[i];
		...
		V[i + dt] = V[i]+ x * S[i]*dt - Betapv * (V[i] - e1 * V[i]) * (Ipu[i] + Ipv[i])*dt - Betamv * (V[i] - e2 * V[i]) * (Imu[i] + Imv[i]) * dt;
       ...
       // Data1 << i << "," << S[i] << "," << V[i] << ", " << Ipu[i] + Ipv[i] << "," << Imu[i] + Imv[i] << "," << Rs[i] + Rv[i] << "," <<Ipu[i] + Ipv[i]+Imu[i] + Imv[i]<< endl;
        ...
        Data1 << Betapv << "," << Betamv << " ," << V[times] << ", " << Rs[times] <<"," <<Rv[times]<< "," << Rs[times]+Rv[times] <<  endl;
        cout << Betapv << "," << Betamv << " ," << V[times] << "," << Rs[times] <<","<<Rv[times]<<"," << Rs[times]+Rv[times] << endl;
		}
